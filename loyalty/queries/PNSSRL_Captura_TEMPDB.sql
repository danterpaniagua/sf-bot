-- Job:      PNSSRL - Captura_TEMPDB
-- Step:     Step1
-- Database: master
-- Status:   enabled
-- Captures: active sessions consuming tempdb space (user_objects + internal_objects)

INSERT INTO PNSSRL.[dbo].[PNSSRL_TempdbProc]
...
SELECT
    ...
    SUBSTRING(
        ...
        END - dmv_er.statement_start_offset) / 2
    )                                                                                               AS Query_Text,
    dmv_tsu.session_id,
    dmv_tsu.request_id,
...
INNER JOIN sys.dm_exec_requests           dmv_er  ON  dmv_tsu.session_id = dmv_er.session_id
...
WHERE (dmv_tsu.internal_objects_alloc_page_count + dmv_tsu.user_objects_alloc_page_count) > 0
  AND SUBSTRING(
        ...
        (CASE
            ...
            ELSE dmv_er.statement_end_offset
        END - dmv_er.statement_start_offset) / 2
      ) NOT LIKE '%SELECT st.dbid AS QueryExecutionContextDBID, DB_NAME(st.dbid) AS QueryExecContextDBNAME%'
  AND SUBSTRING(
        st.TEXT,
        dmv_er.statement_start_offset / 2 + 1,
        (CASE
            ...
            ELSE dmv_er.statement_end_offset
        END - dmv_er.statement_start_offset) / 2
      ) NOT LIKE '%PNSSRL_AuditSysprocesses%'
ORDER BY
