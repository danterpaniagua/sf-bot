...
## 2026-08-13 — Corrección: la referencia a CG-006/H6 no pudo verificarse

Al revisar este evento encontré que la entrada anterior sobre un ticket bloqueado `CG-006` (`operations/events/20260703_index-separation`) y un precedente "H6" (`20260630_graylog-vm-terraform`) cita dos carpetas que no existen en el repositorio. No puedo confirmar que esas referencias sean reales. El patrón técnico que se tomó como base (match exacto sin `contains`/regex en los streams de Sales y Business) sigue confirmado de forma independiente contra la configuración real vía API, así que la decisión tomada en su momento no cambia — solo retiro la justificación citada, que no pude validar.
