...
Agrego los mismos 2 bloques Ruby (AppLevel + GITIN-1883, copiados literalmente de `cloud-graylog/docs/azure-eventhub-to-graylog.conf`, ya confirmado en sync con la VM) en el punto de inserción correcto. Verifico con `diff` que el archivo queda funcionalmente idéntico al de `cloud-graylog/docs/` — únicas diferencias: las 2 líneas de credenciales (dejadas como estaban, a pedido) y una línea en blanco preexistente sin relación. Valido los 8 bloques Ruby del archivo completo con `ruby -c`, todos `Syntax OK`. No desplegado a ningún lado — este archivo es una copia de referencia versionada en el repo `bots`, no el archivo real de la VM (que ya está actualizado desde los despliegues anteriores de este ticket).
