...
- Diseñar, mantener y operar pipelines de CI/CD (GitHub Actions como principal, Jenkins como infraestructura legacy vigente) para despliegues a Azure App Service y otros targets.
...
- Integrar y mantener gates de calidad de código en el pipeline (SonarQube) y estándares de logging estructurado con detección automatizable de violaciones (greps/reglas por axioma, no solo revisión manual).
...
- Colaborar con desarrollo para que cambios de arquitectura (ej. separación de logging, particionado de datos) puedan expresarse como infraestructura/pipeline, no solo como código de aplicación.

## Stack tecnológico

| Categoría | Tecnologías |
|---|---|
| **CI/CD** | GitHub Actions (pipeline principal, despliegues a Azure App Service); Jenkins (infraestructura legacy vigente); SonarQube (gates de calidad) |
...
- Experiencia integrando gates de calidad de código (SonarQube) en pipelines.
